...
describe("Alert Dialog", () => {
  describe("machine", () => {
    it("기본적으로 닫힌 상태로 시작한다", () => {
      const state = machine.initial({ id: "alert-dialog" });

      expect(state).toEqual({ open: false });
    });

    it("defaultOpen이 true이면 열린 상태로 시작한다", () => {
      const state = machine.initial({ id: "alert-dialog", defaultOpen: true });

      expect(state).toEqual({ open: true });
    });

    it("열기 요청을 받으면 연다", () => {
      const props = { id: "alert-dialog" };
    ...
    it("dismiss 요청을 받으면 닫는다", () => {
      const props = { id: "alert-dialog", defaultOpen: true };
      const state = machine.initial(props);
    ...
    it("열림 상태가 바뀌면 변경된 값을 알린다", () => {
      ...
      const props = { id: "alert-dialog", onOpenChange };
      const state = machine.initial(props);
